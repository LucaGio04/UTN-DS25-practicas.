...
function App() {
  ...
  const [nextId, setNextId] = useState(29);
  ...
  const addBook = (newBook) => {
    ...
    setBooksData(prev => ({
      ...prev,
      [newBook.category]: [...(prev[newBook.category] || []), bookWithId]
    }));
  ...
  const removeBook = (bookId, category) => {
    setBooksData(prev => ({
      ...prev,
      [category]: prev[category].filter(book => book.id !== bookId)
    }));
  };
  ...
  const searchResults = searchQuery ? searchBooks(searchQuery) : [];
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <nav>
        <ul>
          ...
          <li><a href="#" onClick={() => { setCurrentPage('catalogo'); setSearchQuery(''); }}>Catálogo</a></li>
          ...
          <li><a href="#" onClick={() => { setCurrentPage('agregar-libro'); setSearchQuery(''); }}>Agregar Libro</a></li>
      ...
      <main className="container">
        ...
        {currentPage === 'catalogo' && <CatalogPage books={searchQuery ? searchResults : allBooks} searchQuery={searchQuery} onRemoveBook={removeBook} />}
        ...
        {currentPage === 'agregar-libro' && <AddBookPage onAddBook={addBook} />}
